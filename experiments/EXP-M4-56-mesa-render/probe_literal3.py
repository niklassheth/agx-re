...
for variant,b6 in [('mode3',v) for v in (0,2,4,6,8,10,12,16,32)]+[('mode2',v) for v in (1,4,8,9,16,17,32,33)]:
 ...
 if variant!='native':code[red+1]=6;code[red+2]=0x54;code[red+3]=b6
 data=bytearray(blob);data[base:base+size]=code+bytes(size-len(code));data[fb:fb+fs]=fragment+bytes(fs-len(fragment))
 name=f'literal3-{variant}-{b6:02x}';Path(name+'.archive').write_bytes(data)
