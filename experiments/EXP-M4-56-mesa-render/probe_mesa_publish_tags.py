 ...
 for tag in (0,1,2):
  ...
  for i in range(len(code)-7):
   if code[i:i+3]==bytes.fromhex('570654'):
    ...
    new+=mov(63,0x3f800000)+bytes([d*16+9,s*2+1,0x35,127,0x41,0,0,(((d//2) if tag==0 else (min(d,5) if tag==1 else (0,1,2,3,4,4,5)[d]))<<2)])
  ...
  name=f'mesapub-tags-{hint}-{tag}';Path(name+'.archive').write_bytes(data)
