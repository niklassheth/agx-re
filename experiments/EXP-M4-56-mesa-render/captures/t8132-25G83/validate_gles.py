...
for vs,fs in [('vertex','fragment')]:
 data=bytearray(blob)
 for stage,name in [('vertex',vs),('fragment',fs)]:
  base,size=agxparse.locate_region(blob,'_agc.main',stage);body=Path('gles-'+name+'.bin').read_bytes();assert len(body)<=size
  data[base:base+size]=body+bytes(size-len(body))
 name=f'validated-gles-{vs}-{fs}';Path(name+'.archive').write_bytes(data)
 ...
 for sx,sy,word in pixels:
  ...
  r=(y+.72)/1.54;g=(1-r+x/.82)/2;b=1-r-g
  if min(r,g,b)>0:
   covered+=1
   if vs=='vertex-w':
...
Path('GLES_VALIDATION.json').write_text(json.dumps(results,indent=2)+'\n')
