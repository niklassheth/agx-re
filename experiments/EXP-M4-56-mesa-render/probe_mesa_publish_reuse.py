...
for d in (4,15):
 for tag in (0,4):
  code=bytearray(Path('mesa-vertex.bin').read_bytes());code[9]|=0x20;new=bytearray();last=0
  for i in range(len(code)-7):
   if code[i:i+3]==bytes.fromhex('570654'):
    new+=code[last:i];s=code[i+3]//2
    new+=mov(63,0x3f800000)+bytes([d*16+9,s*2+1,0x35,127,0x41,0,0,tag<<2])
    export=bytearray(code[i:i+8]);export[3]=d*2;new+=export;last=i+8
  new+=code[last:]
  data=bytearray(blob)
  ...
  name=f'mesapub-reuse-{d}-{tag}';Path(name+'.archive').write_bytes(data)
