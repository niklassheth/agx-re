 ...
 for tag in (0,4):
  ...
  for i in range(len(code)-7):
   if code[i:i+3]==bytes.fromhex('570654'):
    new+=code[last:i];s=code[i+3]//2;d=(code[i+4]>>5)+8
    new+=mov(15,0x3f800000)+bytes([d*16+9,s*2+1,0x35,31,0x41,0,0,tag<<2])
  ...
  name=f'mesapub-upper-{hint}-{tag}';Path(name+'.archive').write_bytes(data)
